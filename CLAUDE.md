...
- Use the newest Laravel syntax valid for this project's Laravel version
...
- On catch, as default action redirect back with an `error` flash message for the frontend toast (if not specified otherwise by specific endpoint logic).
...
- At the end of every reply, append a short trailing section listing what was used to answer the prompt:
  - **Skills:** every skill activated via the Skill tool (or auto-loaded by name match), or `none`.
  - **Agents:** every subagent spawned via the Agent tool with its `subagent_type`, or `none`.
  - **MCP tools:** every MCP tool invoked (the full `mcp__...` name), or `none`.
  - Format as a single block, e.g. `Skills: inertia-vue-development | Agents: Explore | MCP tools: mcp__laravel-boost__search-docs`. Omit built-in tools (Read, Edit, Bash, Grep, Glob, Write, etc.) — only list skills, agents, and MCP tools.
